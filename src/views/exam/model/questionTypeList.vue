...
<script>
import { pageQuestionType, removeQuestionType } from '@/api/exam/questionTypeInfo'
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      pageQuestionType(this.listQuery).then(response => {
    ...
    toCreate() {
      this.$router.push({ path: '/exam/questionTypeAdd' })
    },
    toEdit(id) {
      this.$router.push({ path: '/exam/questionTypeEdit/' + id })
    },
    remove(id) {
      const idVo = {
        id: id
      }
      removeQuestionType(idVo).then(() => {
        this.listLoading = false
        this.reload()
      })
    }
